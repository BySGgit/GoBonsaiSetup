...
import { TreePine, Clock, Target, TrendingUp, Zap, Heart, Sun, Scissors, Droplets, Leaf, ThermometerSnowflake, Waves, Flower2, Download, Upload } from 'lucide-vue-next';
import { computed } from 'vue';

const emit = defineEmits(['reset', 'update-settings', 'update-mode', 'export', 'import']);
...
const props = defineProps<{
  stats: {
    ...
    health: number;
    light: number;
    log: { message: string, time: number, type: 'info' | 'warning' | 'success' }[]
  },
  settings: {
    timeSpeed: number;
    manualLight: boolean;
...
const seasonInfo = computed(() => {
  ...
  if (day >= 60 && day < 150) return { name: 'Весна', icon: Flower2, color: 'text-pink-400' };
  if (day >= 150 && day < 240) return { name: 'Лето', icon: Sun, color: 'text-yellow-400' };
  if (day >= 240 && day < 330) return { name: 'Осень', icon: Leaf, color: 'text-orange-500' };
...
<template>
  <div class="fixed top-6 left-6 z-10 w-64">
    <div class="bg-black/60 backdrop-blur-md border border-white/10 rounded-2xl p-5 text-white shadow-2xl">
      ...
      <div class="grid grid-cols-1 gap-4">
        <div class="flex flex-col gap-1">
          <div class="flex items-center gap-2 text-xs font-medium text-white/50 uppercase tracking-wider">
        ...
        <div class="flex flex-col gap-1">
          ...
          <div class="w-full bg-white/10 h-1.5 rounded-full mt-1 overflow-hidden">
            <div class="bg-orange-400 h-full transition-all duration-300" :style="{ width: `${Math.min(100, stats.light * 100)}%` }"></div>
          </div>
        </div>
        ...
        <div class="flex flex-col gap-1 pt-2 border-t border-white/5">
          <div class="flex items-center justify-between">
            ...
            <span class="text-sm font-mono text-blue-400">{{ stats.targetAge.toFixed(2) }} лет</span>
          </div>
        </div>

        <div class="flex items-center justify-between pt-1">
          <div class="flex items-center gap-2 text-xs font-medium text-white/50 uppercase tracking-wider">
            <component :is="seasonInfo.icon" class="w-3.5 h-3.5" :class="seasonInfo.color" />
            <span>Сезон</span>
          </div>
        ...
        <div class="flex items-center justify-between pt-1">
          ...
          <span class="text-sm font-mono text-amber-400">
            {{ Math.floor(props.stats.gameTime / 365) + 1 }} / {{ Math.floor(props.stats.gameTime % 365) }}
          </span>
        </div>
      </div>
      ...
      <div class="mt-6 pt-4 border-t border-white/10 flex gap-2">
        <button 
          ...
          :class="settings.interactionMode === 'water' ? 'bg-blue-500/40 border-blue-500/50 text-blue-300' : 'bg-white/5 border-white/10 text-white/40'"
          class="flex-1 py-2 rounded-xl border flex flex-col items-center gap-1 transition-all cursor-pointer group"
        >
          <Droplets class="w-4 h-4" :class="settings.interactionMode === 'water' ? 'text-blue-400' : 'group-hover:text-blue-400/60'" />
          <span class="text-[9px] font-bold uppercase tracking-widest">Полив</span>
        </button>
        <button 
          @click="$emit('update-mode', 'prune')"
          :class="settings.interactionMode === 'prune' ? 'bg-orange-500/40 border-orange-500/50 text-orange-300' : 'bg-white/5 border-white/10 text-white/40'"
          class="flex-1 py-2 rounded-xl border flex flex-col items-center gap-1 transition-all cursor-pointer group"
        >
          <Scissors class="w-4 h-4" :class="settings.interactionMode === 'prune' ? 'text-orange-400' : 'group-hover:text-orange-400/60'" />
          <span class="text-[9px] font-bold uppercase tracking-widest">Обрезка</span>
        </button>
        <button 
          @click="$emit('update-mode', 'wire')"
          :class="settings.interactionMode === 'wire' ? 'bg-emerald-500/40 border-emerald-500/50 text-emerald-300' : 'bg-white/5 border-white/10 text-white/40'"
          class="flex-1 py-2 rounded-xl border flex flex-col items-center gap-1 transition-all cursor-pointer group"
        >
          <Waves class="w-4 h-4" :class="settings.interactionMode === 'wire' ? 'text-emerald-400' : 'group-hover:text-emerald-400/60'" />
          <span class="text-[9px] font-bold uppercase tracking-widest">Проволока</span>
        </button>
      </div>
      ...
      <div class="mt-6 pt-4 border-t border-white/10 space-y-4">
        <div class="flex flex-col gap-2">
          ...
          <input 
            ...
            @input="$emit('update-settings', { timeSpeed: parseFloat(($event.target as HTMLInputElement).value) })"
        ...
        <div class="flex flex-col gap-2">
          <div class="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest text-white/40">
            ...
            <button 
              @click="$emit('update-settings', { manualLight: !settings.manualLight })"
              :class="settings.manualLight ? 'text-emerald-400' : 'text-white/20'"
          ...
          <input 
            ...
            @input="$emit('update-settings', { manualLightValue: parseFloat(($event.target as HTMLInputElement).value) })"
      ...
      <!-- Дневник садовода -->
      <div v-if="stats.log && stats.log.length > 0" class="mt-6 pt-4 border-t border-white/10">
        <div class="text-[10px] font-bold uppercase tracking-widest text-white/40 mb-3 flex items-center gap-2">
          <span>Дневник садовода</span>
        </div>
        <div class="space-y-2 max-h-32 overflow-hidden relative">
          <div v-for="(entry, index) in stats.log" :key="entry.time" 
      ...
      <div class="mt-8 pt-4 border-t border-white/10 flex flex-col gap-3">
        ...
        <div class="text-[10px] text-white/30 text-center uppercase tracking-widest font-bold">
